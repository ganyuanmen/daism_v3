...
export default withSession(async (req, res) => {
  ...
  try {  
    ...
    let {rid,pid,id,content} = fields  //rid 修改ID

    if(rid[0]=='0') { //add
      let message_id=uuidv4()
      let rows=await getData("select manager,actor_name,avatar,actor_account,actor_url from a_account where id=?",[id[0]])
      if(rows.length===0){
          res.status(err.httpCode || 500).json({errMsg: "invalid ID"}); 
      }
      else {
          const sql='INSERT INTO a_message_commont(manager,pid,message_id,actor_name,avatar,actor_account,actor_url,content) VALUES(?,?,?,?,?,?,?,?)' 
      ...
      const sql="update a_message_commont set content=? where id=?"
