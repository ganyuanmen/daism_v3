...
export default withSession(async (req, res) => {
  ...
  if(domain!==oldDomain) {   
    ...
    while(true) {
      ...
      let re=await httpGet(`https://${oldDomain}/api/getMessage?account=${oldAccount}&pi=${pi-1}&menutype=${sctype?2:3}&daoid=${daoid}&eventnum=${sctype?0:2}`, {'content-type': 'application/activity+json'});
      console.log('---<',re)
      if(re.code===200){
