...
//id 增加后的自增ID
export function send(account,content,fileName,message_id,title,imgpath,pathtype) 
{
  ...
  getUser('actor_account',account,'privkey,Lower(actor_account) account,actor_name,domain').then(localUser=>{
    try{
        ...
        const thebody=createMessage(localUser.actor_name,localUser.domain,content,fileName,message_id,title,imgpath,process.env.LOCAL_DOMAIN,pathtype);
        getFollowers({account:localUser.account}).then(data=>{
            data.forEach(element => {
                try{
                  if(element.user_inbox.startsWith(`https://${process.env.LOCAL_DOMAIN}`)){
                    insertMessage(id,element.user_account,message_id,pathtype).then(()=>{})
                  }else {
